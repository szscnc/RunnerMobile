...
import { Heart, Zap, Trophy, MapPin, Diamond, Rocket, ArrowUpCircle, Shield, Activity, PlusCircle, Play, ChevronLeft, ChevronRight } from 'lucide-react';
...
const MobileControls: React.FC = () => {
    const { hasImmortality, activateImmortality, isImmortalityActive } = useStore();
    ...
    return (
        <div className="absolute bottom-6 left-0 right-0 px-6 flex justify-between items-end z-[60] pointer-events-auto md:hidden select-none touch-none">
            {/* Left Controls: Movement */}
            <div className="flex gap-3">
                <button 
                    className="w-16 h-16 rounded-full bg-black/40 backdrop-blur-sm border border-cyan-500/50 flex items-center justify-center active:bg-cyan-500/40 active:scale-95 transition-all shadow-[0_0_10px_rgba(0,255,255,0.2)]"
                    onTouchStart={(e) => { e.preventDefault(); dispatchControl('control-left'); }}
                    onMouseDown={() => dispatchControl('control-left')}
                >
                    <ChevronLeft className="w-8 h-8 text-cyan-400" />
                </button>
                <button 
                    className="w-16 h-16 rounded-full bg-black/40 backdrop-blur-sm border border-cyan-500/50 flex items-center justify-center active:bg-cyan-500/40 active:scale-95 transition-all shadow-[0_0_10px_rgba(0,255,255,0.2)]"
                    onTouchStart={(e) => { e.preventDefault(); dispatchControl('control-right'); }}
                    onMouseDown={() => dispatchControl('control-right')}
                >
            ...
            <div className="flex gap-4 items-end">
                {hasImmortality && (
                     <button 
                        className={`w-12 h-12 mb-2 rounded-full border flex items-center justify-center transition-all ${isImmortalityActive ? 'bg-yellow-500/50 border-yellow-400 animate-pulse' : 'bg-black/40 border-yellow-500/50 active:bg-yellow-500/40'}`}
                        onTouchStart={(e) => { e.preventDefault(); activateImmortality(); }}
                        onMouseDown={activateImmortality}
                    >
                ...
                <button 
                    className="w-20 h-20 rounded-full bg-black/40 backdrop-blur-sm border-2 border-pink-500/60 flex items-center justify-center active:bg-pink-500/40 active:scale-95 transition-all shadow-[0_0_15px_rgba(255,0,80,0.3)]"
                    onTouchStart={(e) => { e.preventDefault(); dispatchControl('control-jump'); }}
                    onMouseDown={() => dispatchControl('control-jump')}
                >
                    <ArrowUpCircle className="w-10 h-10 text-pink-400" />
                </button>
            </div>
        </div>
    );
