...
export const Player: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
            ...
            // activateImmortality(); // Handled by buttons mostly now, but tap still works
            // NOTE: disabling direct tap to avoid conflict with virtual buttons if they bubble
        }
    };
  ...
  // Virtual Controls Listener (from HUD)
  useEffect(() => {
      const handleControlLeft = () => {
          if (status === GameStatus.PLAYING) setLane(l => Math.max(l - 1, -Math.floor(laneCount / 2)));
      };
      const handleControlRight = () => {
          if (status === GameStatus.PLAYING) setLane(l => Math.min(l + 1, Math.floor(laneCount / 2)));
      };
      const handleControlJump = () => {
           if (status === GameStatus.PLAYING) triggerJump();
      };

      window.addEventListener('control-left', handleControlLeft);
      window.addEventListener('control-right', handleControlRight);
      window.addEventListener('control-jump', handleControlJump);
      
      return () => {
          window.removeEventListener('control-left', handleControlLeft);
          window.removeEventListener('control-right', handleControlRight);
          window.removeEventListener('control-jump', handleControlJump);
      }
